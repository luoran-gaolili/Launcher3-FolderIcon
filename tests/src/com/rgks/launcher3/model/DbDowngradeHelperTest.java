...
public class DbDowngradeHelperTest {
    ...
    public void testDowngrade_success_v22() throws Exception {
        ...
        try (Cursor c = helper.getWritableDatabase().query(LauncherSettings.Favorites.TABLE_NAME,
                null, null, null, null, null, null)) {
            assertEquals(-1, c.getColumnIndex(LauncherSettings.Favorites.OPTIONS));
        ...
        helper = new LauncherProvider.DatabaseHelper(mContext, null, DB_FILE) {
            @Override
        ...
        try (Cursor c = helper.getWritableDatabase().query(LauncherSettings.Favorites.TABLE_NAME,
                null, null, null, null, null, null)) {
            // Check column exists
            assertNotSame(-1, c.getColumnIndex(LauncherSettings.Favorites.OPTIONS));
    ...
    private void setupTestDb() throws Exception {
        ...
        LauncherProvider.DatabaseHelper dbHelper = new LauncherProvider.DatabaseHelper(mContext, null, DB_FILE) {
            @Override
            public void onOpen(SQLiteDatabase db) { }
        };
        // Insert dummy data
        for (int i = 0; i < 10; i++) {
            ...
            values.put(LauncherSettings.Favorites.TITLE, "title " + i);
            dbHelper.getWritableDatabase().insert(LauncherSettings.Favorites.TABLE_NAME, null, values);
        }
        dbHelper.close();
    }
