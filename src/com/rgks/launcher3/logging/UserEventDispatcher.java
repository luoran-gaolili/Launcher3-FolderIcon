...
import com.rgks.launcher3.util.LogConfig;
import com.rgks.launcher3.userevent.nano.LauncherLogProto;

import java.util.Locale;
import java.util.UUID;

import static com.rgks.launcher3.logging.LoggerUtils.newItemTarget;
import static com.rgks.launcher3.logging.LoggerUtils.newTarget;
...
public class UserEventDispatcher {
    ...
    public static UserEventDispatcher newInstance(Context context, boolean isInLandscapeMode,
        ...
        UserEventDispatcher ued = Utilities.getOverrideObject(UserEventDispatcher.class,
                context.getApplicationContext(), com.rgks.launcher3.R.string.user_event_dispatcher_class);
    ...
    public void logAppLaunch(View v, Intent intent) {
        LauncherLogProto.LauncherEvent event = LoggerUtils.newLauncherEvent(LoggerUtils.newTouchAction(LauncherLogProto.Action.Touch.TAP),
                LoggerUtils.newItemTarget(v), LoggerUtils.newTarget(LauncherLogProto.Target.Type.CONTAINER));
    ...
    public void logActionCommand(int command, int containerType, int pageIndex) {
        LauncherLogProto.LauncherEvent event = LoggerUtils.newLauncherEvent(
                LoggerUtils.newCommandAction(command), LoggerUtils.newContainerTarget(containerType));
    ...
    public void logActionCommand(int command, View itemView, int containerType) {
        LauncherLogProto.LauncherEvent event = LoggerUtils.newLauncherEvent(LoggerUtils.newCommandAction(command),
                LoggerUtils.newItemTarget(itemView), LoggerUtils.newTarget(LauncherLogProto.Target.Type.CONTAINER));
    ...
    public void logActionOnControl(int action, int controlType, @Nullable View controlInContainer) {
        final LauncherLogProto.LauncherEvent event = controlInContainer == null
    ...
    public void logDeepShortcutsOpen(View icon) {
        ...
        LauncherLogProto.LauncherEvent event = LoggerUtils.newLauncherEvent(LoggerUtils.newTouchAction(LauncherLogProto.Action.Touch.LONGPRESS),
                LoggerUtils.newItemTarget(info), LoggerUtils.newTarget(LauncherLogProto.Target.Type.CONTAINER));
    ...
    * care about which screen moves to where. */
    ...
    public void logDragNDrop(DropTarget.DragObject dragObj, View dropTargetAsView) {
        LauncherLogProto.LauncherEvent event = LoggerUtils.newLauncherEvent(LoggerUtils.newTouchAction(LauncherLogProto.Action.Touch.DRAGDROP),
                LoggerUtils.newItemTarget(dragObj.originalDragInfo), LoggerUtils.newTarget(LauncherLogProto.Target.Type.CONTAINER));
        event.destTarget = new LauncherLogProto.Target[] {
                LoggerUtils.newItemTarget(dragObj.originalDragInfo), LoggerUtils.newDropTarget(dropTargetAsView)
        };
