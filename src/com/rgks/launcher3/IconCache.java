...
import com.rgks.launcher3.util.InstantAppResolver;
...
public class IconCache {
    ...
    private boolean isDynamCalender =false; //Add by luoran for 8.1 Launcher dynamic calendar 20180111
    public IconCache(Context context, InvariantDeviceProfile inv) {
        ...
        isDynamCalender = context.getResources().getBoolean(R.bool.support_calendar_icon);
    ...
    @Thunk synchronized void addIconToDBAndMemCache(LauncherActivityInfo app,
        ...
        if (entry == null) {
            ...
            //Add by luoran for 8.1 Launcher dynamic calendar 20180111(start)
            if(isDynamCalender && app.getComponentName().equals("com.android.calendar")) {
                entry.icon = LauncherIcons.createCalendarIconBitmap(getFullResIcon(app), mContext);
            }else {
    ...
    boolean isCalenderInfo;//Add by luoran for 8.1 Launcher dynamic calendar 20180111
    /**
    ...
    protected CacheEntry cacheLocked(
        ...
        if (componentName != null){
            isCalenderInfo = componentName.getPackageName().equals("com.android.calendar");
        }
        //Add by luoran for 8.1 Launcher dynamic calendar 20180111(end)
        if (isCalenderInfo || entry == null || (entry.isLowResIcon && !useLowResIcon)) {
            ...
            //Add by luoran for 8.1 Launcher dynamic calendar 20180111(end)
            if (isCalenderInfo || !getEntryFromDB(cacheKey, entry, useLowResIcon) || DEBUG_IGNORE_CACHE) {
                info = infoProvider.get();
                providerFetchedOnce = true;

                if (info != null) {
                    if (isCalenderInfo && isDynamCalender) {
                        entry.icon = LauncherIcons.createCalendarIconBitmap(info.getIcon(mIconDpi), mContext);
                    } else{
            ...
            if (!getEntryFromDB(cacheKey, entry, useLowResIcon)) {
                try {
                    ...
                    if(isDynamCalender && packageName.equals("com.android.calendar")) {
                        icon = LauncherIcons.createCalendarIconBitmap(appInfo.loadIcon(mPackageManager), mContext);
                    }else {
