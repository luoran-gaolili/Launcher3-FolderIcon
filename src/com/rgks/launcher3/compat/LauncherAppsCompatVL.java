...
import com.rgks.launcher3.compat.ShortcutConfigActivityInfo.ShortcutConfigActivityInfoVL;
...
public class LauncherAppsCompatVL extends LauncherAppsCompat {
    ...
    //add by luoran for hide app(start)
    private List<String> hideappsArrayList = new ArrayList<String>();
    private List<LauncherActivityInfo> allFilterAppsList = new ArrayList<>();
    //add by luoran for hide app(end)
    ...
    public List<LauncherActivityInfo> getActivityList(String packageName, UserHandle user) {
        //modify by luoran for hide app(start)
        hideappsArrayList = Arrays.asList(mContext.getResources().getStringArray(R.array.hide_apps_arraylist));
        allFilterAppsList.clear();
        if (hideappsArrayList != null && hideappsArrayList.size() > 0) {
            List<LauncherActivityInfo> allAppsList = mLauncherApps.getActivityList(packageName, user);
            for (LauncherActivityInfo launcherActivityInfo : allAppsList) {
                if (!(hideappsArrayList.contains(launcherActivityInfo.getApplicationInfo().packageName))) {
                    allFilterAppsList.add(launcherActivityInfo);
                }
            }
            Log.d("LUORAN", "allFilterAppsList:" + allFilterAppsList.size());
            return allFilterAppsList;
        } else {
            return mLauncherApps.getActivityList(packageName, user);
        }
        //modify by luoran for hide app(end)
    }
