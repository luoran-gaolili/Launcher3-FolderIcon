...
import static com.rgks.launcher3.AutoInstallsLayout.beginDocument;
...
public class MTKUnreadLoader extends BroadcastReceiver {
    ...
    //add by luoran for modify google mms unread support(start)
    ComponentName MmscomponentName;
    //add by luoran for modify google mms unread support(end)
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        if (FeatureFlags.ACTION_UNREAD_CHANGED.equals(action)) {
            //modify by luoran for modify google mms unread support(start)
            final ComponentName componentName = (ComponentName) intent
                    .getExtras().get(FeatureFlags.EXTRA_UNREAD_COMPONENT);
            Log.d("LUORAN","onReceive" + componentName);
            /*if(componentName.equals(MmscomponentName)){
                componentName = new ComponentName("com.google.android.apps.messaging",
                        "com.google.android.apps.messaging.ui.ConversationListActivity");
            }*/
            //modify by luoran for modify google mms unread support(end)
            final int unreadNum = intent.getIntExtra(FeatureFlags.EXTRA_UNREAD_NUMBER, -1);
